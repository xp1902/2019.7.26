...
<head>
   ...
   function onQuery(vaab101,vaab103)
   {
 	 ...
 	 vform.action="<%=path%>/applyForElder.html?aab101="+vaab101+"&aab103="+encodeURI(vaab103);
 	 vform.submit();
   }
   window.onload=function()
   {
	   if(!${empty msg}) alert("申请成功");
	   var a=${empty rows};
		if(a)
		{
			var vform = document.getElementById("myform");
			vform.action="<%=path%>/queryElderA.html?aaj101=${empty ins.aaj101?sessionScope.aaj101:ins.aaj101}";
		 	vform.submit();
		}
   }
   </script>
	   ...
	   <c:choose>
	     <c:when test="${rows!=null }">
		     <c:forEach items="${rows }" var="ins" varStatus="vs">
				    ...
				    <td>${ins.aab114 }</td>
				    <td>
				      <a href="#" onclick="onQuery('${ins.aab101}','${ins.aab103 }')">为该老人申请用品</a>
				    </td>
				  </tr>
		      </c:forEach>
